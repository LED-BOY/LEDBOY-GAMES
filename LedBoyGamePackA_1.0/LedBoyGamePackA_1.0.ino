...
#define RESET_MASK1 1<<1
#define RESET_MASK2 1<<2
#define RESET_MASK3 1<<3
...
void loop() {
  ...
  if ((millis() - timer) > 10000 && shouldWakeUp) { // sleep code while in clock mode
    ...
    shouldWakeUp = false;
    ADC0.CTRLA  &= ~ ADC_ENABLE_bm; //adc disable to save some power
...
void gameSelection() {
  ...
  while (digitalRead(RIGHT) == LOW && digitalRead(LEFT) == LOW); //prevents immediately enter a game

  while (gameSelect < 4) {

    if (buttonDebounce(0, 1, 0, 350)) {
      gameSelect++;
    ...
    switch (gameSelect) { // select game ad scroll game title
      case 0:
        ...
        for (uint8_t x = 0; x < 5 && digitalRead(RIGHT) == HIGH && digitalRead(LEFT) == HIGH && digitalRead(SLEEPMENU) == HIGH; x++) {
          switch (x) {
            case 1:
        ...
        if (digitalRead(SLEEPMENU) == LOW || (digitalRead(LEFT) == LOW && digitalRead(RIGHT) == LOW)) {
          gameSelect = 4;
          gameMusic();
      ...
      case 1:
        ...
        for (uint8_t x = 0; x < 4 && digitalRead(RIGHT) == HIGH && digitalRead(LEFT) == HIGH && digitalRead(SLEEPMENU) == HIGH; x++) {
          switch (x) {
            case 1:
              scrollPixels(ledTrisTitle1, 50, 80, 0);
              break;
            case 2:
        ...
        for (uint8_t x = 0; x < 4 && digitalRead(RIGHT) == HIGH && digitalRead(LEFT) == HIGH && digitalRead(SLEEPMENU) == HIGH; x++) {
          switch (x) {
            ...
            case 2:
              ...
              break;
            case 3:
              scrollPixels(flappyLedTitle3, 0, 100, 50);
              break;
          }
        }
...
void gameLedTris() {
  ...
  while (ledtrisLives > 0 && !endGame) {
    ...
    if (tetrominoRotation != tetrominoLastRotation && sound) {
      beep(40, 40);
    }
    ...
    if (tetrominoStop) {//All calcultions done when a tetromino stops moving
      ...
      if (digitalRead(RIGHT) == HIGH && digitalRead(LEFT) == HIGH || tetrominoTimer > 400) {//Some conditions have to be met before stoping so some time is given to last moment moves
        beep(50, 150);
        ...
        for ( uint8_t repeatTetrisCheck = 0; repeatTetrisCheck < 3; repeatTetrisCheck++ ) {
          ...
          for ( uint8_t row = 9; row > 0; row-- ) {
            ...
            if (tetris) {
              ...
              if (actualLedTrisSpeed > 200) { // make game faster
                actualLedTrisSpeed -= 10;
              }
...
void gameLedInvaders () {
  ...
  while (enemiesRemaining > 0 && ledInvaderLives > 0) {
    ...
    if (enemyShotTimer >= 300) {// enemiees fire code
      enemyShotTimer = 0;
      strip.setPixelColor(enemyShotPosition, 0 , 0 , 0);

      if (enemyShotPosition > 99) {
        enemyShotPosition = actualEnemies[random(arrayElements)]; // select enemy that is going to fire

        if  (enemyShotPosition != enemiesKilled [enemyShotPosition]) {// check if the enemy is killed it doesn´t fire.
          enemyShotPosition += enemyPos;
        } else {
...
void gameMenu () {// menu only available in game mode
  ...
  while (option < 3) {
    ...
    while (digitalRead(RIGHT) == LOW || digitalRead(LEFT) == LOW); // button debounce
...
ISR(RTC_PIT_vect) {// rtc interupt
  ...
  if ((!(VPORTA.IN & RESET_MASK1)) && (!(VPORTA.IN & RESET_MASK2)) && (!(VPORTA.IN & RESET_MASK3))) { // if all 3 button are pressed, reset.
