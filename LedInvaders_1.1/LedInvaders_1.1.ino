...
void loop() {
  ...
  if (seconds % 2 == 0) {
    battStatus();// show batt status on even secods
  }
...
void gameMenu () {// menu only available in game mode
  ...
  while (option != 3) {
    ...
    if (buttonDebounce(0, 1, 0, 500) && option < 2) {
      option++;
      strip.clear();
    }

    if (buttonDebounce(1, 0, 0, 500) && option > 0) {
      option--;
      strip.clear();
    }
...
void battStatus() { // show batt status color coded green is full, red empty
  voltageReading = map(readSupplyVoltage(), 2950, 3700, 0, 2);
