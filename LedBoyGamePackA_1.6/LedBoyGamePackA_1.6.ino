...
void setup() {
  ...
  oldState = ((!(PORTA.IN & PIN5_bm)) | !(PORTA.IN & PIN6_bm)  << 1);
...
void loop() {
  ...
  if (LEFTLOW && RIGHTLOW) { // Clock always on function
  ...
  if (buttonDebounce(0, 0, 1, 600)) { // enters menu options
    setOptions();
  ...
  while (gameSelection) {
    ...
    switch (gameSelect) { // select game ad scroll game title
      ...
      case 3:
        ...
        if (SLEEPMENULOW || (LEFTLOW && RIGHTLOW)) {
          encoderSetPosition(1, 1);
          strip.clear();
          while (RIGHTLOW && LEFTLOW); //prevents immediately enter a game

          while (RIGHTHIGH || LEFTHIGH) {
            encoderRead(1, 2, true, 1);
            arrayNumberValue[0] = pgm_read_word_near(number + rotaryCounter);
            ...
            if (LedKanoidNumberOfplayers != rotaryCounter) {
              strip.clear();
              beep(50, 50);
              LedKanoidNumberOfplayers = rotaryCounter;
            }
          }
          gameSelection = false;
        }
...
void gameLedKanoid() {
  ...
  encoderSetPosition (playerOnePos, 2);
  ...
  while (ledKanoidLives != 0 && blocksRemaining != 0) {
    ...
    encoderRead(90, 97, false, 2);
    playerOnePos = rotaryCounter;
    ...
    if (buttonDebounce(0, 0, 1, 300)) {//Pause menu
      gameMenu ();
      for (uint8_t x = 0; x < arrayElements; x++) {
        if (x != blocksDestroyed [x]) {
          strip.setPixelColor(actualBlocks[x], 100, 0, 0);// led enemy position musn´t be less than 0
        }
      }
    }
  ...
  if (ledKanoidLives == 0) {// Game over
    gameLevel = 1;

    if (ledKanoidScore > 999) {
      ledKanoidScore = 999;
    }
...
void gameFlappyLed() {
  ...
  while (flappyLedLives != 0) {
    ...
    if (flappyLedTimer >= 2500 && flappyLedPos < 93) {
      flappyLedTimer = 0;
      flappyLedPos += 10;
    }
...
void gameLedTris() {
  ...
  while (ledtrisLives > 0 && !endGame) {
    ...
    if (tetrominoPos > 0) {
      ...
      if (buttonDebounce(1, 0, 0, 120) && RIGHTHIGH && tetrominoMoveLeft) {
        tetrominoPos--;
      }

      if (buttonDebounce(0, 1, 0, 120) && LEFTHIGH && tetrominoMoveRight) {
        tetrominoPos++;
      }
    }
