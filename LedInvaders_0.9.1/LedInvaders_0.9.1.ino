...
void game () {
  ...
  if (enemiesRemaining == 0) {// go to next level
    ...
    if (gameLevel > 9) {// for now  game continues only to level 9
...
void gameMenu () {// menu only available in game mode
  ...
  while (option != 3) {
    ...
    if (buttonDebounce(0, 1, 0, 250) && option < 2) {
      option++;
      strip.clear();
    }

    if (buttonDebounce(1, 0, 0, 250) && option > 0) {
      option--;
      strip.clear();
    }
