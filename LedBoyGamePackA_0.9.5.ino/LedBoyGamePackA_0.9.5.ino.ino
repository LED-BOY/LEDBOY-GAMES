...
void gameFlappyLed() {
  ...
  while (flappyLedLives != 0) {
    ...
    if (wallPos == 3 && (flappyLedPos < passage1 || flappyLedPos > (passage2 + 14))) {
      flappyLedLives = 0;
      for (uint16_t x = 0; x < 800; x += 100) {// game music
        beep(200, x);
      }
    }
...
void gameLedTris() {
  ...
  while (ledtrisLives > 0 && !endGame) {
    ...
    if (tetrominoStop) {//All calcultions done when a tetromino stops moving
      ...
      if (digitalRead(RIGHT) == HIGH && digitalRead(LEFT) == HIGH  || tetrominoTimer > 350) {//Some conditions have to be met before stoping so some time is given to last moment moves
        beep(100, 250);
