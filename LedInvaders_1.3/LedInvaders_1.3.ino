...
void gameMenu () {// menu only available in game mode
  ...
  while (option != 3) {
    ...
    if (buttonDebounce(0, 1, 0, 50) && option < 2) {
    ...
    if (buttonDebounce(1, 0, 0, 50) && option > 0) {
...
void battStatus() { // show batt status color coded green is full, red empty
  voltageReading = map(readSupplyVoltage(), 2980, 3720, 0, 9);
  if (voltageReading > 5) {
