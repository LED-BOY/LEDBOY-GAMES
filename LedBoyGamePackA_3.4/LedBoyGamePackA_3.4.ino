...
uint16_t buttonLeftTimeStamp; // button press time
uint16_t buttonRightTimeStamp; // button press time
uint16_t buttonMenuTimeStamp; // button press time
...
void loop() {
  ...
  if (buttonDebounce(1, 0, 0)) clockAlwaysOn (); // Clock always on function

  if (buttonDebounce(0, 1, 0)) { // enters game mode
    strip.clear();
    gameSelection();
  }
...
void gameSelection() {
  ...
  while (gameSelection) {
    ...
    if (buttonDebounce(1, 1, 0)) gameSelection = false;
...
void gameLedKanoid() {
  ...
  while (ledKanoidLives != 0 && blocksRemaining != 0) {
    ...
    if (LedKanoidNumberOfplayers == 2) {
      ...
      if (buttonDebounce(1, 0, 0) && playerTwoPos > 0) {
        playerTwoPos--;
      }

      if (buttonDebounce(0, 1, 0) && playerTwoPos < 7) {
        playerTwoPos++;
      }
    ...
    if (buttonDebounce(1, 1, 0) && ballOnPlatform) {
      ballMovement = true;
    ...
    if (buttonDebounce(0, 0, 1)) {//Pause menu
      gameMenu ();
      for (uint8_t x = 0; x < arrayElements; x++) {
  ...
  while (!buttonDebounce(1, 1, 0)) {
    ...
    if (buttonDebounce(0, 0, 1) && buttonDebounce(0, 1, 0)) {
      strip.clear();
    ...
    if ((interruptTimer - generalTimer) > 10000) {// if no button is pressed in 30sec ends game
      contGame = false;
      endGame = true;
      break;
    }
...
void gameFlappyLed() {
  ...
  while (flappyLedLives != 0) {
    ...
    if (flappyLedPos > 3 && (LEFTLOW || RIGHTLOW) && (interruptTimer - buttondebounce) > 80) {
    ...
    if (buttonDebounce(0, 0, 1)) gameMenu ();//Pause menu
...
void gameLedTris() {
  ...
  while (ledtrisLives > 0 && !endGame) {
    ...
    if ((interruptTimer > ledTrisSpeed) && (tetrominoPos < 99) && !tetrominoStop) {
      interruptTimer = 0;
      tetrominoPos += 10;
    }
    ...
    if (tetrominoStop) {//All calcultions done when a tetromino stops moving
      ...
      if (buttonDebounce(1, 1, 0) || interruptTimer > 400) {//Some conditions have to be met before stoping so some time is given to last moment moves
        beep(150);
    ...
    if (tetrominoPos > 0) {
      ...
      if (buttonDebounce(1, 0, 0) && tetrominoMoveLeft)  tetrominoPos--;

      if (buttonDebounce(0, 1, 0) && tetrominoMoveRight) tetrominoPos++;
    }
...
void gameLedInvaders () {
  ...
  while (enemiesRemaining > 0 && ledInvaderLives > 0) {
    ...
    if (buttonDebounce(0, 0, 1)) gameMenu();     //enemiesRemaining = 0;// uncoment to cheat and advance level with one button ;)
...
void gameMenu () {// menu only available in game mode
  ...
  while (option < 3) {
    ...
    if (buttonDebounce(0, 1, 0)) {
      if (option++, option > 2) option = 0;
      strip.clear();
    }

    if (buttonDebounce(1, 0, 0)) {
...
bool buttonDebounce(bool left, bool right, bool menu) {// code to debounce button press

  if (rightButtonPressed && right) {
    buttonRightTimeStamp = interruptTimer;
    rightButtonPressed = false;
    return true;
  } else if (!RIGHTLOW && (interruptTimer - buttonRightTimeStamp) > 50) { // some debounce time
    PORTA.PIN1CTRL |= PORT_ISC_FALLING_gc;
  }
  ...
  if (leftButtonPressed && left) {
    ...
    return true;
  } else if (!LEFTLOW && (interruptTimer - buttonLeftTimeStamp) > 50) {
    PORTA.PIN2CTRL |= PORT_ISC_FALLING_gc;
  }
  ...
  if (!menuButtonPressed && menu && SLEEPMENULOW) {
    ...
    return true;
  } else if (!SLEEPMENULOW && (interruptTimer - buttonMenuTimeStamp) > 50) {
  ...
  while (option < 5) {
    ...
    switch (option) { // color NeoPixel LED number from first to last
      ...
      case 4:
        ...
        if (buttonDebounce (0, 0, 1)) option = 5; // end options
        break;
    }
...
void beep(uint16_t delay) { // sound function
  ...
  for (uint16_t x = 0; x <= delay; x++) {
    PORTB.OUT |= PIN1_bm; // buzzer pin high
    __asm__("nop\n\t");
  }
...
ISR(TCB0_INT_vect) {// anoter interrupt to show pixels on screen, because micros are lost using RTC and .show cna´t be called one after another  this is the best way I found to call "strip.show"
  ...
  if (interruptTimer % 20 == 0) strip.show(); // 50 frames a second aprox
  interruptTimer ++;
  encoderTimer++;
}
